...
const changeFirstLeterToUpperCase = text => {
...
const DueItems = ({navigation, items, i, setSelectedDueInvoice}) => {
  ...
  const getOverDuePeriod = invoiceDueDate => {
    ...
    const difference = todayMoment.diff(invoiceDueDateMoment, 'days');
    return difference > 0 ? difference.toString().replace('-', '') : 0;
  };
    ...
    <View key={item.key} style={styles.tileListingContainer}>
          ...
          <View style={styles.textContainer}>
            <View style={styles.duration}>
              <Text style={styles.tileDueText}>
                {moment(item.invoice_issued_date).format('MMM YYYY')}
                {'-'}
                {moment(item.invoice_due_date).format('MMM YYYY')}
              </Text>
            </View>
            <View style={styles.tilesDueTypeContainer}>
              <Text style={styles.tileDueTypeText}>
                {`${changeFirstLeterToUpperCase(item.fee_duration)} invoice`}
              </Text>
              <Text style={styles.tileDuePriceText}>
                {item.total_amount}
                {' LKR'}
              </Text>
            </View>
      ...
      <View style={styles.detailSecondRowContainer}>
        <View style={styles.tileLeftSide}>
          <Text style={styles.overDueText}>
            Overdue for {getOverDuePeriod(item.invoice_due_date)} days
          </Text>
        </View>
        <View style={styles.tileRightSide}>
      ...
      <View style={styles.detailThirdRowContainer}>
        <Animated.View
          ...
          {invoiceItems.map((costItem, id) => (
            <View key={id} style={styles.showMoreContainer}>
              <Text style={styles.paymentSectionText}>
                {changeFirstLeterToUpperCase(costItem.cost_item_name)}
              </Text>
              <Text style={styles.paymentSectionText}>
                {costItem.cost}
                {' LKR'}
          ...
          <View style={styles.showMoreContainer}>
            <Text style={styles.paymentSectionText}>Apartner Charges</Text>
            <Text style={styles.paymentSectionText}>
              {item.apartner_charges}
              {' LKR'}
            </Text>
          </View>
        </Animated.View>
